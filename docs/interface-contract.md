...
```jsonc
{
  ...
  "rootVel": [0.1, 0.0, -0.05], // [可选 v1.1] 髋中点速度(米/秒),canonical 系;方案 C 根运动
  "grounded": true,          // [可选 v1.1] 双脚是否贴地;缺省视为 true(消费端保守回退)
...
- `conf` 长度 = 骨骼数,缺省视为全 1。
- `rootVel`(可选,v1.1 起)3 个 `number`(canonical 米/秒),由 `root-motion.js` 的
  `RootMotionTracker` 对髋中点相邻帧差分求出,用于还原跳跃/前后左右移动。
- `grounded`(可选,v1.1 起)boolean,由「最低脚踝相对地面估计」判断;旧序列无此字段,
  消费端回退到纯「脚贴地」运动学(向后兼容)。
- **评分只消费 `bones`/`conf`**;`rootVel`/`grounded`/`rootYaw` 仅供可视化根运动,
  不进入相似度(玩家站哪、跳多高不影响「动作像不像」)。
